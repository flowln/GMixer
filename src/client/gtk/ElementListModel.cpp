...
ElementListModel* ElementListModel::create(ElementType type)
{
    return new ElementListModel(type);
}

ElementListModel::ElementListModel(ElementType type) : m_type(type)
{
    m_store = Gtk::ListStore::create(m_records);
}

void ElementListModel::populate(bool blocking)
{
    if (isPopulated)
        return;

    if (!blocking) {
        std::thread populate([this] { populateInternal(); });
        populate.detach();
    } else {
        populateInternal();
    }
...
void ElementListModel::populateInternal()
{
